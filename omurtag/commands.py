...
from .utils import get_data_directory, create_directory
...
def run(args: Namespace):
    ...
    match args.mode:
        ...
        case 'pull':
            print("[yellow]'pull' is not yet implemented, coming soon![/yellow]")
        case _:
            print(args.mode)
...
def _ensure_template_exists(args, data_dir, template:str, print_flag=True)-> bool:
...
def _delete(args, data_dir):
    ...
    if not _ensure_template_exists(args, data_dir,
                                  template=to_be_deleted,
                                  print_flag=True):
        return
...
def _create(args, data_dir):
    ...
    if project_path.exists():
        if not Confirm.ask(f"'{project_path}' already exists. Overwrite?",
                           default=False):
    ...
    try:
        copytree(src, dst, dirs_exist_ok=True)
        print(f"[green]Project '{project_path.name}' created at {project_path.parent}[/green]")
    except PermissionError:
